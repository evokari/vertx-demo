...
import com.juhaevokari.op.pac.config.DbConfig;
...
public class FlywayMigration {
  ...
  public static Future<Void> migrate(final Vertx vertx, final DbConfig dbConfig){
    ...
    return vertx.<Void>executeBlocking(promise -> {
      execute(dbConfig);
      promise.complete();
    }).onFailure(err -> LOG.error("Failed to migrate the DB schema with error: ", err));
  }
  ...
  private static String printMigrations(MigrationInfo[] pendingMigrations) {
    ...
    return Arrays.stream(pendingMigrations)
      .map(migration -> migration.getVersion() + " - " + migration.getDescription())
